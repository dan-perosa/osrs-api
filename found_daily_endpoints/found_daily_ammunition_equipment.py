...
def found_daily_ammunition_equipment():
    # q = select(Equipment).where(Equipment.slot == 'ammunition')
    # ammunition_equipment_list_length = len(session.execute(q).scalars().all())
    # random_equipment_number = random.randint(1, ammunition_equipment_list_length)
    q = select(Equipment).where(Equipment.slot == 'Ammunition').order_by(func.random()).limit(1)
