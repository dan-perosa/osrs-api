...
def found_daily_hands_equipment():
    # q = select(Equipment).where(Equipment.slot == 'Hands')
    # hands_equipment_list_length = len(scoped_session_to_use.execute(q).scalars().all())
    q = select(Equipment).where(Equipment.slot == 'Hands')
