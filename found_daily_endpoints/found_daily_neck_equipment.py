...
def found_daily_neck_equipment():
    # q = select(Equipment).where(Equipment.slot == 'Neck')
    # neck_equipment_list_length = len(session.execute(q).scalars().all())
    # random_equipment_number = random.randint(1, neck_equipment_list_length)
    q = select(Equipment).where(Equipment.slot == 'Neck').order_by(func.random()).limit(1)
